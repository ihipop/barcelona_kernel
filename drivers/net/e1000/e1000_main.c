...
#ifdef CONFIG_ARCH_GEN3
static enum fake_phy_mode  check_fake_phy(void)
{
	u32 phy_base; /* L2 switch bar 0 */
	u8 __iomem *virt_base;
	u32 reg_val; /* slave config register */
	struct pci_dev *pdev;
	enum fake_phy_mode ret = NO_FAKE_PHY;
...
#ifdef CONFIG_ARCH_GEN3
	if(hw->revision_id == 0x8)
		hw->fake_phy_mode = check_fake_phy();
	else
		hw->fake_phy_mode = NO_FAKE_PHY;
#endif
...
#ifdef CONFIG_ARCH_GEN3
	if (hw->fake_phy_mode == FAKE_PHY_EXTERNAL)
		hw->phy_addr = MARVAL_PHY_ADDRESS;
#endif	
